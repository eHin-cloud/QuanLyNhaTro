...
<head>
    ...
    <title>Đăng ký chủ trọ - SmartRoom</title>
...
<body class="min-h-screen bg-[#080b11] text-slate-100 selection:bg-indigo-500 selection:text-white">
    <main class="min-h-screen grid grid-cols-1 lg:grid-cols-2">
        <section class="hidden lg:flex flex-col justify-between p-10 bg-slate-950 border-r border-slate-900">
            ...
            <div class="max-w-xl">
                <p class="text-xs font-bold uppercase tracking-[0.25em] text-emerald-300">Onboarding cho chủ trọ</p>
                <h1 class="mt-4 text-4xl font-black tracking-tight text-white">Vào dashboard trước, xác minh sau khi cần nhận tiền.</h1>
                <p class="mt-5 text-sm leading-7 text-slate-400">
                    Bạn chỉ cần số điện thoại, tên nhà trọ và địa chỉ để bắt đầu thêm phòng. Hồ sơ CCCD, ngân hàng, PCCC và giấy phép sẽ được nhắc đúng thời điểm, không làm bạn bị ngợp ngay từ đầu.
                </p>
            </div>

            <div class="grid grid-cols-3 gap-3 text-xs">
                <div class="rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
                    <strong class="block text-emerald-300">Bước 1</strong>
                    <span class="text-slate-500">Tạo nhà trọ</span>
                </div>
                <div class="rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
                    <strong class="block text-amber-300">Bước 2</strong>
                    <span class="text-slate-500">KYC khi có tiền</span>
                </div>
                <div class="rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
                    <strong class="block text-sky-300">Bước 3</strong>
                    <span class="text-slate-500">Tích xanh</span>
                </div>
            </div>
        </section>
        ...
        <section class="flex items-center justify-center px-5 py-10">
            <div class="w-full max-w-xl">
                <div class="mb-8">
                    <p class="text-xs font-bold uppercase tracking-[0.2em] text-indigo-300">Đăng ký nhanh</p>
                    <h2 class="mt-3 text-2xl font-black text-white">Tạo tài khoản chủ trọ</h2>
                    <p class="mt-2 text-sm text-slate-500">OTP demo hiện tại: <strong class="text-slate-300">123456</strong></p>
                </div>
                ...
                <form method="POST" action="{{ route('landlord.register.store') }}" class="space-y-4 rounded-3xl border border-slate-800 bg-slate-900/45 p-6 shadow-2xl shadow-black/20">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <label class="block">
                            <span class="block text-xs font-bold text-slate-400 mb-2">Số điện thoại</span>
                    ...
                    <label class="block">
                        <span class="block text-xs font-bold text-slate-400 mb-2">Họ và tên</span>
                    ...
                    <label class="block">
                        <span class="block text-xs font-bold text-slate-400 mb-2">Tên nhà trọ mong muốn</span>
                        <input name="property_name" value="{{ old('property_name') }}" required maxlength="160" placeholder="VD: Nhà trọ Minh Anh" class="w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-sm text-slate-100 outline-none focus:border-indigo-500">
                    </label>

                    <label class="block">
                        <span class="block text-xs font-bold text-slate-400 mb-2">Địa chỉ nhà trọ</span>
                    ...
                    <label class="block">
                        <span class="block text-xs font-bold text-slate-400 mb-2">Mật khẩu</span>
                    ...
                    <div class="rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-xs leading-6 text-amber-100">
                        Phòng bạn đăng sẽ có nhãn "Chưa xác minh" trên Renty. Khi cần nhận tiền tự động, hệ thống mới yêu cầu CCCD và tài khoản ngân hàng.
                    </div>
                    ...
                    <button type="submit" class="w-full rounded-xl bg-indigo-600 px-5 py-3 text-sm font-black text-white hover:bg-indigo-500 active:scale-[0.99] transition">
                        Tạo dashboard chủ trọ
                    </button>

                    <div class="text-center text-xs text-slate-500">
                        Đã có tài khoản? <a href="{{ route('login') }}" class="font-bold text-indigo-300 hover:text-indigo-200">Đăng nhập</a>
                    </div>
                </form>
            </div>
        </section>
