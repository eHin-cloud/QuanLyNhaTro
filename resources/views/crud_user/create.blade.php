...
<head>
    ...
    <title>Đăng Ký - SmartRoom & Renty</title>
    ...
    <main class="flex-grow flex items-center justify-center px-4 py-8 relative z-10">
        <div class="w-full max-w-md bg-slate-900/40 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-[0_20px_50px_rgba(0,0,0,0.3)] hover:border-slate-700/60 transition-all duration-300">
            <div class="text-center mb-8">
                <h1 class="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-white via-slate-200 to-slate-400 bg-clip-text text-transparent mb-2">Đăng Ký Tài Khoản</h1>
                <p class="text-xs text-slate-400">Tạo tài khoản quản lý căn hộ dịch vụ mới của bạn</p>
            </div>

            <form action="{{ route('user.postUser') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="name" id="name" required class="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-800/80 rounded-xl text-slate-200 text-sm focus:border-indigo-500 focus:outline-none transition-all" placeholder="Nguyễn Văn A">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="like" id="like" required class="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-800/80 rounded-xl text-slate-200 text-sm focus:border-indigo-500 focus:outline-none transition-all" placeholder="Quản lý chung cư mini">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="password" name="password" id="password" required class="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-800/80 rounded-xl text-slate-200 text-sm focus:border-indigo-500 focus:outline-none transition-all" placeholder="•••••••• (tối thiểu 6 ký tự)">
                    </div>
                </div>

                <button type="submit" class="w-full mt-2 py-3 px-4 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-semibold text-sm shadow-lg shadow-indigo-500/20 transform active:scale-95 transition-all">
                    Đăng Ký Tài Khoản
                </button>
            </form>

            <div class="mt-8 pt-6 border-t border-slate-800/60 text-center">
                <p class="text-xs text-slate-400 mb-3">Đã có tài khoản quản lý?</p>
                <a href="{{ route('login') }}" class="inline-block px-4 py-2 rounded-xl bg-slate-950 border border-slate-800 hover:border-slate-700 text-xs font-semibold text-indigo-400 transition-all">
                    Đăng Nhập Ngay
                </a>
            </div>
        </div>
